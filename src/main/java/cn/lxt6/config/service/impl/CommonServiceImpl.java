package cn.lxt6.config.service.impl;


import cn.lxt6.config.core.annotation.Auto;
import cn.lxt6.config.core.annotation.bean.Service;
import cn.lxt6.config.dao.ICommonDao;
import cn.lxt6.config.service.ICommonService;
...
public class CommonServiceImpl implements ICommonService {
    ...
    @Override
    public <T, Bean> T insert(Bean bean) {
        Map<String, Object> par = JsonUtil.model2Model(bean, Map.class).get();
        T id = commonDao.insert(par, TableUtil.getTableName(bean.getClass()));
        return id;
    }

    @Override
    public <Bean> Boolean update(Bean setBean, Bean whereBean) {
        Map<String, Object> wherePar = JsonUtil.model2Model(whereBean, Map.class).get();
        Map<String, Object> setPar = JsonUtil.model2Model(setBean, Map.class).get();
    ...
    public <Bean> Bean getOnePart(Bean whereBean, String... columns) {
        ...
        return JsonUtil.map2Model(beanMap, beanClass).get();
    }
    ...
    public <Bean> Boolean delete(Bean bean) {
        Map<String, Object> wherePar = JsonUtil.model2Model(bean, Map.class).get();
